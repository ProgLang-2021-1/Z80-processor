...
def call():
	...
	Debug().newFunction('CALL {:02X}{:02X}H'.format(val2,val1))
def ret():
	Z80().offsetPC()
...
def jump_call():
	...
	if match := re.search(JR_E, '{0:08b}'.format(Bus().data)):
		...
		if ((match.group('cc') == FlagCC.NZ and Z80().getFlag(FlagCC.NZ.name)) or
		(match.group('cc') == FlagCC.Z and Z80().getFlag(FlagCC.Z.name)) or
		(match.group('cc') == FlagCC.NC and Z80().getFlag(FlagCC.NC.name)) or
		(match.group('cc') == FlagCC.C and Z80().getFlag(FlagCC.C.name)) or
		(match.group('cc') == FlagCC.PO and Z80().getFlag(FlagCC.PO.name)) or
		(match.group('cc') == FlagCC.PE and Z80().getFlag(FlagCC.PE.name)) or
